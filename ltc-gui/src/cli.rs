...
fn resolve_device(devices: &[audio_core::AudioDeviceInfo], cli: &Cli) -> Result<String, String> {
    if let Some(ref id) = cli.device {
        ...
        return Err(format!("Device '{}' not found. Use --list-devices to see available devices.", id));
    }
    ...
    // Default: use first device that is marked as default
    for dev in devices {
        if dev.is_default {
            return Ok(dev.id.clone());
        }
...
pub fn run_headless(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !valid_fps.iter().any(|f| (f - fps).abs() < 0.01) {
        return Err(format!("Unsupported fps: {}. Must be one of: 24, 25, 29.97, 30", fps).into());
    }

    let valid_channels = ["left", "right", "both"];
    if !valid_channels.iter().any(|c| *c == channel) {
        return Err(format!("Unsupported channel: '{}'. Must be one of: left, right, both", channel).into());
    }
    ...
    info!("Headless mode: tc={}, fps={}, drop_frame={}, channel={}, volume={}",
        timecode_fmt(&start_tc), fps, drop_frame, channel, volume);
    ...
    // Verbose: print starting timecode
    if verbose {
        println!("LTC {}  |  {} Hz  |  {} fps{}  |  {:?}",
    ...
    while running.load(Ordering::SeqCst) {
        ...
        // Verbose: print timecode changes
        if verbose {
    ...
    let _ = core.stop_ltc();
    // Capture final TC before stop_output() drops the audio state
    ...
    if verbose {
        println!("{}", "-".repeat(60));
        println!("Done.  Duration: {:.1}s  |  Frames: {}  |  Final TC: {}",
            elapsed.as_secs_f64(),
            total_frames,
    ...
    info!("Headless mode complete: duration={:.2}s, frames={}", elapsed.as_secs_f64(), total_frames);
    Ok(())
}
...
pub fn generate_wav(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let total_samples = total_frames as usize * samples_per_frame;
    let num_channels = 2;
    ...
    let file_size_kb = (total_samples * 2 * 2) as f64 / 1024.0; // 16-bit * 2 channels
    if cli.verbose {
        println!();
    }
    println!(
        "WAV written: {}  |  {} frames  |  {:.1}s  |  {:.0} KB  |  {} Hz  |  {} channels",
        path, total_frames, duration_secs, file_size_kb, sample_rate, num_channels
    );
